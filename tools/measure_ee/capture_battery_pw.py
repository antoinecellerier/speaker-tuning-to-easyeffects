...
Sibling to `capture_battery.py` (which captures the live EasyEffects
chain). Both write `loopback_<stim>_<label>.{wav,json}` in the same
schema so `tools/measure_dax/analyze.py` and the comparison tooling
...
Pre-flight:
  ...
  4. Wire the chain's playback into the null sink:
        pw-link effect_output.<NAME>:output_FL ee_capture:playback_FL
        pw-link effect_output.<NAME>:output_FR ee_capture:playback_FR
  5. Then run this script.

Example:

    python3 tools/measure_ee/capture_battery_pw.py \\
...
DEFAULT_OUT_DIR = REPO_ROOT / "localresearch" / "measure_ee" / "captures_pw"
...
def main() -> int:
    ...
    print("next: tools/measure_ee/compare_ee_vs_pw.py to overlay against EE")
    return 0
