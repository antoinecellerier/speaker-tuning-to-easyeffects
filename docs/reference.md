...
| `tests/` · `tools/` · `docs/` | suite, measurement harnesses, these notes |
...
`register-python-argcomplete` line, the emitted `_generator` and
`# Generated by` provenance stamps, and the two weekly workflows that rewrite
tables inside the generator all name them — so they stay put. Any module that
is *not* an entry point belongs in `lib/`, which
