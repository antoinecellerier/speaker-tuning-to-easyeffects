...
2. *New in-house captures (X1 Yoga, HDA).* **Linux side done 2026-06-12:**
   ...
   entries 6/11). **Windows side still pending:** DAX captures of speech
   (DE-on vs DE-off profile contrast), decorrelated stereo, and the
   stepped/loud battery at pinned master volume. Tooling:
   `tools/measure_dax/` (Windows capture) and `tools/measure_ee/` (Linux
   loopback).
