...
2. *New in-house captures (X1 Yoga, HDA).* The Linux-side EE captures must be
   ...
   comparison; capture `stimulus_stepped_loud` (−2 dBFS peak — crosses the
   ≈ −6.4 dBFS MBC knee the −18 dBFS battery never reached) to wake the
   MBC / regulator out of dormancy (entries 6, 11); capture the stereo
   stimuli on `dynamic`/`movie` for the surround mapping (entry 2) and
   `stimulus_speech` for the dialog enhancer (entry 1). Tooling:
