...
``.github/workflows/speaker-quirks.yml``, which writes both quirk tables. Each
runs a deterministic ``tools/update_*.py`` script that locates a dict literal
by regex, splices a re-rendered body in, and opens a PR. That is a safe thing
to do to a file whose whole content is the table, and an uncomfortable one to
do inside a script somebody is editing by hand the same week.
