...
- **`coupled-bands`.** The engaged path has been neither DAX-captured nor heard
  ([docs/reference.md](docs/reference.md)). The per-band limiter covers these
  zones by default.
...
- **`high-shelf`.** The reproduction of the Dolby tuning is numerically
  ...
  [docs/cross-device-findings.md](docs/cross-device-findings.md#type-3--high-shelf-filter-experimental)
  lists where the type-3 shelf appears.
- **`lo-pass`.**
  [docs/cross-device-findings.md](docs/cross-device-findings.md#types-6-and-8--low-pass-variants-experimental)
...
- **`virtual-bass`.** Windows DAX synthesizes harmonics that suggest bass small
  ...
  parameters so the
  [PipeWire converter](#pipewire-filter-chain-instead-of-easyeffects) can build
